...
static void postCannyFilter(const Mat& src, Mat& dx, Mat& dy, int low, int high, Mat& dst)
{
    ...
    // L2 Gradient comparison with square
    high = high * high;
    low = low * low;
    ...
    vector<Contour>& contours, OutputArray hierarchy, int mode)
{
    ...
    contoursInPixel.erase(std::remove_if(contoursInPixel.begin(), contoursInPixel.end(), [](const std::vector<cv::Point>& cnt) {
...
// New function extract contour vector<point> & remove short contour
vector<Point2f> extractSubPixCnt(Mat& gray, double alpha, int low, int high) {
    Mat d;
    getCannyKernel(d, alpha);
    Mat one = Mat::ones(Size(1, 1), CV_16S);
    Mat dx, dy;
    sepFilter2D(gray, dx, CV_16S, d, one);
    sepFilter2D(gray, dy, CV_16S, one, d);
    ...
    // contours in pixel precision
    vector<vector<Point> > contoursInPixel;
    findContours(edge, contoursInPixel, RETR_TREE, CHAIN_APPROX_NONE);
    ...
    vector<Point2f> contour_output;
    for (const auto& contour : contours) {
        for (const auto& cnt : contour.points) {
            contour_output.push_back(cv::Point2f(cnt.x, cnt.y));
        }
    }
...
// New function converting InPixel contour to SubPixel contour
vector<Point2f> InPix2SubPix(Mat& gray, vector<vector<Point>> contour, double alpha) {
    ...
    // subpixel position extraction with steger's method and facet model 2nd polynominal in 3x3 neighbourhood
    vector<Contour> contours;
    extractSubPixPoints(dx, dy, contour, contours);
    // Concatenate contours (a vector of vectors containing points) into contour (a vector containing points)
    vector<Point2f> contour_output;
    for (const auto& contour : contours) {
        for (const auto& cnt : contour.points) {
